...
int main(int argc, char * argv[]) {
	...
	if(rank==0) { // This code is run by a single process
		...
		for ( i = 1; i < nProcesses; i++){
			MPI_Send( &base_val , 1, MPI_INT, i , 0 , MPI_COMM_WORLD);
		}
		...
		int i;
		int *v = (int*)malloc(sizeof(int) * (nProcesses - 1));
		int *newV = (int*)malloc(sizeof(int) * (nProcesses - 1));
		MPI_Recv(&actual_value , 1 , MPI_INT , 0 , 0 , MPI_COMM_WORLD,NULL);
		MPI_Recv(v, nProcesses-1, MPI_INT, rank-1 , 0 , MPI_COMM_WORLD, NULL);
		for(i = rank-1; i < nProcesses-1; i++) {
			if(actual_value == -1) {
				actual_value = v[i];
		...
		if(rank < nProcesses-1){
			MPI_Send(newV , nProcesses-1, MPI_INT, rank+1 , 0 , MPI_COMM_WORLD);
